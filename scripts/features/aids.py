...
def process_patient(chunk):
    chunk['aids'] = chunk.icd_code.isin(icd_filter).astype(int)
    ...
    chunk = chunk[~chunk.index.duplicated(keep='last')] # Sometimes an admission will have 0 then 1 if there was another diagnosis prior to aids
    return chunk

if __name__ == '__main__':
    print('Saving aids feature...')
    main().to_pickle(feature_root + '/aids.pickle')
    print('Saved aids!\n')
