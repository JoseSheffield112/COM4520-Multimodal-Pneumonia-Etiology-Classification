...
def main():
    ...
    if save_intermediates:
        ...
        intermediate_path = Path(intermediate_root + '/crp.csv')  
        intermediate_path.parent.mkdir(parents=True, exist_ok=True) 
        data.to_csv(intermediate_path)
        print('Saved intermediate crp!')
...
def process_admission(chunk):
    ...
    if len(x) >= MIN_READINGS_TO_KEEP:
        first['crp'] = [np.interp(range(24), x, y).round(1)]
    else:
        first['crp'] = [np.nan]
    return first

if __name__ == '__main__':
    print('Saving crp feature...')
    main().to_pickle(feature_root + '/crp.pickle')
    print('Saved crp!\n')
