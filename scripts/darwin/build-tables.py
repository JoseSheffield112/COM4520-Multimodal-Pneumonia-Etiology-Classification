...
def get_icd_dict():
    return {
        ...
        'chartevents'   : [723, 454, 184, 223900, 223901, 220739, 51, 442, 455, 6701, 220179, 220050, 211, 220045, 678, 223761, 676, 223762, 223835, 3420, 3422, 190, 227444, 227456, 227457],
...
def build_patients(columns=None, samplesize=None, cohort=False):
    ...
    iter_csv = pd.read_csv(root_mimiciv + '/core/patients.csv', header=0, index_col=[0], iterator=True, chunksize=1000, usecols=columns)
    ...
    output_path = Path(out_stripped_mimic_root + '/core/patients.csv')
...
def build_admissions(subject_ids=None, columns=None, cohort=False):
    ...
    iter_csv = pd.read_csv(root_mimiciv + '/core/admissions.csv', header=0, index_col=[0], iterator=True, chunksize=1000, usecols=columns)
    if cohort:
    ...
    output_path = Path(out_stripped_mimic_root + '/core/admissions.csv')  
    output_path.parent.mkdir(parents=True, exist_ok=True) 
    data.to_csv(output_path)
...
def build_diagnoses_icd(subject_ids=None, hadm_ids=None, columns=None):
    ...
    iter_csv = pd.read_csv(root_mimiciv + '/hosp/diagnoses_icd.csv', header=0, index_col=[0], iterator=True, chunksize=1000, usecols=columns)
    if hadm_ids.any():
    ...
    print('Read origin')
    output_path = Path(out_stripped_mimic_root + '/hosp/diagnoses_icd.csv')  
    output_path.parent.mkdir(parents=True, exist_ok=True) 
    data.to_csv(output_path)
...
def build_labevents(subject_ids=None, hadm_ids=None, columns=None, itemid_filter=None):
    ...
    iter_csv = pd.read_csv(root_mimiciv + '/hosp/labevents.csv', header=0, index_col=[0], iterator=True, chunksize=10000, usecols=columns)
    if hadm_ids.any() and itemid_filter:
        ...
        data = pd.concat([chunk for chunk in iter_csv])
    print('Read origin')
    output_path = Path(out_stripped_mimic_root + '/hosp/labevents.csv')  
    output_path.parent.mkdir(parents=True, exist_ok=True) 
    data.to_csv(output_path)
...
def build_services(subject_ids=None, hadm_ids=None, columns=None):
    ...
    iter_csv = pd.read_csv(root_mimiciv + '/hosp/services.csv', header=0, index_col=[0], iterator=True, chunksize=1000, usecols=columns)
    if hadm_ids.any():
        data = pd.concat([chunk[(chunk.index.isin(subject_ids)) | (chunk.hadm_id.isin(hadm_ids))] for chunk in iter_csv])
    else:
        data = pd.concat([chunk for chunk in iter_csv])
    print('Read origin')
    output_path = Path(out_stripped_mimic_root + '/hosp/services.csv')  
    output_path.parent.mkdir(parents=True, exist_ok=True) 
    data.to_csv(output_path)
...
def build_icustays(subject_ids=None, hadm_ids=None, columns=None):
    ...
    iter_csv = pd.read_csv(root_mimiciv + '/icu/icustays.csv', header=0, index_col=[0], iterator=True, chunksize=1000, usecols=columns)
    if hadm_ids.any():
        data = pd.concat([chunk[(chunk.index.isin(subject_ids)) | (chunk.hadm_id.isin(hadm_ids))] for chunk in iter_csv])
    else:
        data = pd.concat([chunk for chunk in iter_csv])
    print('Read origin')
    output_path = Path(out_stripped_mimic_root + '/icu/icustays.csv')  
    output_path.parent.mkdir(parents=True, exist_ok=True) 
    data.to_csv(output_path)
...
def build_chartevents(subject_ids=None, hadm_ids=None, stay_ids=None, columns=None, itemid_filter=None):
    ...
    iter_csv = pd.read_csv(root_mimiciv + '/icu/chartevents.csv', header=0, index_col=[0], iterator=True, chunksize=10000, usecols=columns)
    if stay_ids.any() and itemid_filter:
        ...
        data = pd.concat([chunk for chunk in iter_csv])
    print('Read origin')
    output_path = Path(out_stripped_mimic_root + '/icu/chartevents.csv')  
    output_path.parent.mkdir(parents=True, exist_ok=True) 
    data.to_csv(output_path)
...
def build_outputevents(subject_ids=None, hadm_ids=None, stay_ids=None, columns=None, itemid_filter=None):
    ...
    iter_csv = pd.read_csv(root_mimiciv + '/icu/outputevents.csv', header=0, index_col=[0], iterator=True, chunksize=10000, usecols=columns)
    if stay_ids.any() and itemid_filter:
        data = pd.concat([chunk[((chunk.index.isin(subject_ids)) | (chunk.hadm_id.isin(hadm_ids)) | (chunk.stay_id.isin(stay_ids))) & (chunk.itemid.isin(itemid_filter))] for chunk in iter_csv])
    elif stay_ids.any():
        data = pd.concat([chunk[chunk.stay_id.isin(stay_ids)] for chunk in iter_csv])
        ...
        data = pd.concat([chunk for chunk in iter_csv])
    print('Read origin')
    output_path = Path(out_stripped_mimic_root + '/icu/outputevents.csv')  
    output_path.parent.mkdir(parents=True, exist_ok=True) 
    data.to_csv(output_path)
