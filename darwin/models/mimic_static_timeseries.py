...
def get_encoders_head_fusion(static_output_size,dropout,dropoutP):
    ...
    head = MLP(static_output_size + (30*24), 40, 2, dropout=False).cuda()
    fusion = Concat().cuda()
    ...
    if (kfold < 2):
        ...
        for i in range(nrRuns):
            traindata, validdata, testdata = get_dataloader(
                batch_size, imputed_path=config.dataPath, model = const.Models.static_timeseries,shuffle_split = shuffle_split)
        ...
        for i in range(nrRuns):
            train_val_splits = get_dataloader(
                batch_size, imputed_path=config.dataPath, model = const.Models.static_timeseries,shuffle_split = shuffle_split,kfold=kfold)
            allAcc = []
            for s_i,(traindata,testdata) in enumerate(train_val_splits):

                encoders, head, fusion = get_encoders_head_fusion(static_output_size,dropout,dropoutP,timeseries_hid_layer_size)
                # train
