...
def runModel(nrRuns,outputRoot,nrEpochs,shuffle_split = True,lr =0.001,dropout=False,dropoutP=0.1,optimizer=torch.optim.RMSprop,earlyStop = False,kfold = 0,augmentImages = True,batch_size = 3):
    ...
    if (kfold < 2):
        ...
        pd.DataFrame(test_accuracies,columns=['acc']).to_csv(outputRoot + "/{}-test-kfold_acc.csv".format(MODEL_NAME))
...
if __name__ == '__main__':
    freeze_support()
    runModel(nrRuns = 5,outputRoot = config.stats_root,nrEpochs = 20,kfold=1,optimizer=torch.optim.Adam, lr=0.0001)