    ...
    if (kfold < 2):
        ...
        for i in range(nrRuns):
            traindata, validdata, testdata = get_dataloader(
                batch_size, imputed_path=config.dataPath, model = const.Models.static,shuffle_split = shuffle_split)
        ...
        for i in range(nrRuns):
            train_val_splits = get_dataloader(
                batch_size, imputed_path=config.dataPath, model = const.Models.static,shuffle_split = shuffle_split,kfold=kfold)
            allAcc = []
            ...
            print("Average of best accuracy on testing of each split: {}\n".format(avgAcc))
