...
def runModel(nrRuns,outputRoot,nrEpochs,shuffle_split = True,lr = 0.001,
    ...
    if (kfold < 2):
        ...
        for i in range(nrRuns):
            traindata, validdata, testdata = get_dataloader(
                batch_size, imputed_path=config.dataPath, model = const.Models.timeseries,shuffle_split = shuffle_split)
        ...
        for i in range(nrRuns):
            train_val_splits = get_dataloader(
                batch_size, imputed_path=config.dataPath, model = const.Models.timeseries,shuffle_split = shuffle_split,kfold=kfold)
            allAcc = []
    ...
    with open(outputRoot + "/model_arhitecture.txt", 'w') as f:
        f.write('Timeseries output layer: ' + str(24*30) + '\n')
