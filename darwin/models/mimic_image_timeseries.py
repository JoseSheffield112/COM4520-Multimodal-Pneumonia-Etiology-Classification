...
def get_encoders_head_fusion():
    ...
    head = MLP(const.image_encoder_output_size + (30*24), 40, 2, dropout=False).cuda()
    fusion = Concat().cuda()
    ...
    if (kfold < 2):
        ...
        for i in range(nrRuns):
            traindata, validdata, testdata = get_dataloader(
                batch_size, imputed_path=config.dataPath, model = const.Models.timeseries_image,shuffle_split = shuffle_split,augment_images=augmentImages)
        
            encoders, head, fusion = get_encoders_head_fusion()
        ...
        for i in range(nrRuns):
            train_val_splits = get_dataloader(
                batch_size, imputed_path=config.dataPath, model = const.Models.timeseries_image,shuffle_split = shuffle_split,kfold=kfold,augment_images=augmentImages)
            allAcc = []
