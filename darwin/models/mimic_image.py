...
def runModel(nrRuns,outputRoot,nrEpochs,shuffle_split = True,lr =0.001,dropout=False,dropoutP=0.1,
            optimizer=torch.optim.RMSprop,earlyStop = False,kfold = 0,augmentImages = True,batch_size = 3,save_models=False,patience=7,
    ...
    if (kfold < 2):
        ...
        for i in range(nrRuns):
            traindata, validdata, testdata = get_dataloader(
                batch_size, imputed_path=config.dataPath, model = const.Models.image,shuffle_split = shuffle_split,augment_images=augmentImages)

            encoders, head, fusion = get_encoders_head_fusion(static_output_size,dropout,dropoutP)

            # train
            stats,model,bestacc = train(encoders, fusion, head, traindata, validdata, nrEpochs, auprc=True,lr = lr,
            early_stop=earlyStop,optimtype=optimizer,max_patience=patience,early_stop_metric=early_stop_metric)
        ...
        avg_test_accuracies = []
        for i in range(nrRuns):
            train_test_splits = get_dataloader(
                batch_size, imputed_path=config.dataPath, model = const.Models.image,shuffle_split = shuffle_split,kfold=kfold,augment_images=augmentImages)
            bestModel = None
            allAcc = []
            for s_i,(traindata,testdata) in enumerate(train_test_splits):

                encoders, head, fusion = get_encoders_head_fusion(static_output_size,dropout,dropoutP)
                # train
                stats,model,bestacc = train(encoders, fusion, head, traindata, testdata, nrEpochs, auprc=True,
            ...
            avgAcc = sum(allAcc) / len(train_test_splits)
            avg_test_accuracies.append(avgAcc)
            print("Average of best accuracy on validation of each split: {}\n".format(avgAcc))
        
        #Output average val accuracies
        pd.DataFrame(avg_test_accuracies,columns=['acc']).to_csv(outputRoot + "/{}-kfold-avgvalidacc.csv".format(MODEL_NAME))
