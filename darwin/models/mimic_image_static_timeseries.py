...
def get_encoders_head_fusion(static_output_size,dropout,dropoutP):
    ...
    encoders = [MLP(const.nr_static_features, 50, static_output_size, dropout=dropout,dropoutp=dropoutP).cuda(),
                GRU(const.nr_timeseries_features, 30, dropout=False, batch_first=True).cuda(),
                image_model.cuda()]
    head = MLP(static_output_size + (30*24) + const.image_encoder_output_size, 40, 2, dropout=False).cuda()
    fusion = Concat().cuda()
...
def runModel(nrRuns,outputRoot,nrEpochs,shuffle_split = True,lr =0.001,dropout=False,dropoutP=0.1,
    ...
    MODEL_NAME = "static_timeseries_image"
    ...
    if (kfold < 2):
        ...
        for i in range(nrRuns):
            traindata, validdata, testdata = get_dataloader(
                batch_size, imputed_path=config.dataPath, model = const.Models.static_timeseries_image,shuffle_split = shuffle_split,augment_images=augmentImages)
        ...
        for i in range(nrRuns):
            train_val_splits = get_dataloader(
                batch_size, imputed_path=config.dataPath, model = const.Models.static_timeseries_image,shuffle_split = shuffle_split,kfold=kfold,augment_images=augmentImages)
            allAcc = []
            for s_i,(traindata,testdata) in enumerate(train_val_splits):
                ...
                print("Best accuracy on testing for this split: {}\n".format(bestacc))

                # Output results of this split
                if not os.path.exists(outputRoot + "/run-{}".format(i)):
                    os.makedirs(outputRoot + "/run-{}".format(i))
                outputStats(stats,outputRoot, "/run-{}/split-{}-{}-testing.csv".format(str(i),s_i, MODEL_NAME))
